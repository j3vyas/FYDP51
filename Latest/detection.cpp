...
bool Detection::isRectangle(vector<Point> contour) {
	...
	vector<Point> approx;
	approxPolyDP(contour, approx, 0.07 * perim, true);
	if (approx.size() == 4) {
...
void Detection::showFilteredImg(Mat img, Object obj) {
	if (obj.getColour() == "red") {
		imshow("red filtered", img);
	}

	if (obj.getColour() == "green") {
		imshow("green filtered", img);
	}

	if (obj.getColour() == "yellow") {
		imshow("yellow filtered", img);
	}

	if (obj.getColour() == "blue") {
		imshow("blue filtered", img);
	}
}
...
void Detection::sqlStatement(time_t timeIn, int sessionID, string &query) {
	...
	string tmpSession = to_string(sessionID).c_str();
	cout << sessionID << endl;
	sql += tmpSession;
	sql += ", '";
	sql += timeBuffer;
	...
	string timeDif = to_string(timeDiff).c_str();
	cout << timeDiff << endl;
	sql += timeDif;
	sql += ", '";
...
int Detection::detectionFunc() {
	...
	cout << timeIn << "," << sessionID << "," << sql << endl;
	...
	while (true) {
		...
		getObjects(hsvImg, ref(blue), ref(points));
		getObjects(hsvImg, ref(green), ref(points));
		getObjects(hsvImg, ref(yellow), ref(points));
		getObjects(hsvImg, ref(red), ref(points));
	...
	while (true) {
		...
		int numPointsFound = getNumPointsFound(green, blue, red, yellow);
